...
namespace winsetup::adapters::platform {
    ...
    namespace {
        ...
        std::wstring TrimString(std::wstring str) {
            str.erase(str.begin(),
                std::find_if(str.begin(), str.end(), [](wchar_t ch) { return !std::iswspace(static_cast<wint_t>(ch)); }));
            str.erase(
                std::find_if(str.rbegin(), str.rend(), [](wchar_t ch) { return !std::iswspace(static_cast<wint_t>(ch)); }).base(),
                str.end());
            return str;
        }
    ...
    domain::Expected<void> SMBIOSParser::Initialize() {
        ...
        if (bufferSize == 0)
            return domain::Error(L"Failed to get SMBIOS table size",
                GetLastError(), domain::ErrorCategory::System);

        mRawData.resize(static_cast<size_t>(bufferSize));

        DWORD result = GetSystemFirmwareTable('RSMB', 0, mRawData.data(), bufferSize);
        if (result == 0) {
            mRawData.clear();
            return domain::Error(L"Failed to read SMBIOS table",
                GetLastError(), domain::ErrorCategory::System);
        }
    ...
    domain::Expected<SMBIOSInfo> SMBIOSParser::GetSystemInfo() {
        if (!mParsed) {
            ...
            if (!initResult.HasValue()) return initResult.GetError();
        }
        SMBIOSInfo info;
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetBIOSVersion() {
        ...
        if (mBIOSInfo.version.empty()) return std::wstring(L"Unknown");
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetBIOSVendor() {
        ...
        if (mBIOSInfo.vendor.empty()) return std::wstring(L"Unknown");
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetMotherboardManufacturer() {
        ...
        if (mBaseboardInfo.manufacturer.empty()) return std::wstring(L"Unknown");
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetMotherboardProduct() {
        ...
        if (mBaseboardInfo.product.empty()) return std::wstring(L"Unknown");
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetMotherboardModel() {
        ...
        if (model.empty()) return std::wstring(L"Unknown Motherboard");
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetSystemManufacturer() {
        ...
        if (mSystemInfo.manufacturer.empty()) return std::wstring(L"Unknown");
    ...
    domain::Expected<std::wstring> SMBIOSParser::GetSystemProductName() {
        ...
        if (mSystemInfo.productName.empty()) return std::wstring(L"Unknown");
    ...
    domain::Expected<void> SMBIOSParser::ParseTables() {
        if (mRawData.size() < sizeof(RawSMBIOSData))
            return domain::Error(L"Invalid SMBIOS data",
                ERROR_INVALID_DATA, domain::ErrorCategory::System);

        const auto* rawTable = reinterpret_cast<const RawSMBIOSData*>(mRawData.data());
        const uint8_t* tableData = rawTable->SMBIOSTableData;
        const uint8_t* tableEnd = mRawData.data() + mRawData.size();
        const uint8_t* currentPtr = tableData;

        while (currentPtr + sizeof(SMBIOSHeader) <= tableEnd) {
            const auto* header = reinterpret_cast<const SMBIOSHeader*>(currentPtr);
            if (header->Length < sizeof(SMBIOSHeader)) break;
            if (currentPtr + header->Length > tableEnd)  break;
            ...
            while (currentPtr + 1 < tableEnd) {
                if (currentPtr[0] == 0 && currentPtr[1] == 0) { currentPtr += 2; break; }
                currentPtr++;
            }
