...
namespace winsetup::adapters::platform {
    ...
    domain::Expected<void> Win32SystemInfoService::EnsureSMBIOSInitialized() {
        ...
        if (!initResult.HasValue()) {
            if (m_logger) {
                ...
                m_logger->Warning(L"Failed to initialize SMBIOS parser: "
                    + error.GetMessage());
            }
            return initResult;
        }
    ...
    domain::Expected<std::wstring> Win32SystemInfoService::GetMotherboardModel() {
        ...
        if (!initResult.HasValue()) {
            if (m_logger) {
                m_logger->Warning(L"SMBIOS not available, cannot retrieve motherboard model");
            }
            return initResult.GetError();
        }

        auto modelResult = m_smbiosParser->GetMotherboardProduct();
    ...
    domain::Expected<std::wstring> Win32SystemInfoService::GetBIOSVersion() {
        ...
        if (!initResult.HasValue()) {
            if (m_logger) {
                m_logger->Warning(L"SMBIOS not available, cannot retrieve BIOS version");
            }
            return initResult.GetError();
        }
    ...
    domain::Expected<bool> Win32SystemInfoService::IsUEFIBoot() {
        ...
        if (!result) {
            ...
            if (m_logger) {
                m_logger->Warning(L"Failed to get firmware type, error code: "
        ...
        if (m_logger) {
            m_logger->Info(isUEFI ? L"System is UEFI" : L"System is Legacy BIOS");
        }
    ...
    domain::Expected<uint64_t> Win32SystemInfoService::GetTotalMemoryBytes() {
        ...
        if (!GlobalMemoryStatusEx(&memStatus)) {
            DWORD error = GetLastError();
            if (m_logger) {
                m_logger->Error(L"Failed to get memory status, error code: "
                    + std::to_wstring(error));
            }
            return domain::Error(
