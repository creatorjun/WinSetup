...
namespace winsetup::adapters::platform {
    ...
    domain::Expected<void> Win32SystemInfoService::EnsureSMBIOSInitialized() {
        if (m_smbiosInitialized) {
            return domain::Expected<void>();
        }

        if (!m_smbiosParser) {
            m_smbiosParser = std::make_unique<SMBIOSParser>();
        }
        ...
        if (!initResult.HasValue()) {
            if (m_logger) {
                ...
                m_logger->Warning(
                    L"Failed to initialize SMBIOS parser: " +
                    error.GetMessage()
                );
            }
            return initResult;
        ...
        if (m_logger) {
            m_logger->Debug(L"SMBIOS parser initialized successfully");
        }
    ...
    domain::Expected<std::wstring> Win32SystemInfoService::GetMotherboardModel() {
        ...
        if (!initResult.HasValue()) {
            if (m_logger) {
                m_logger->Warning(L"SMBIOS not available, returning fallback value");
            }
        ...
        if (!modelResult.HasValue()) {
            if (m_logger) {
                ...
                m_logger->Warning(
                    L"Failed to get motherboard model: " +
                    error.GetMessage()
                );
            }
            return std::wstring(L"Unknown Motherboard");
        ...
        if (m_logger) {
            m_logger->Debug(L"Motherboard model: " + model);
        }
    ...
    domain::Expected<std::wstring> Win32SystemInfoService::GetBIOSVersion() {
        ...
        if (m_logger) {
            m_logger->Debug(L"BIOS version: " + version);
        }
    ...
    domain::Expected<bool> Win32SystemInfoService::IsUEFIBoot() {
        ...
        if (!result) {
            ...
            if (m_logger) {
                m_logger->Warning(
                    L"Failed to get firmware type, error code: " +
                    std::to_wstring(error)
                );
            }
            return domain::Error{
                L"Failed to determine firmware type",
                error,
        ...
        if (m_logger) {
            m_logger->Info(
                isUEFI ? L"System is UEFI" : L"System is Legacy BIOS"
            );
        }
    ...
    domain::Expected<uint64_t> Win32SystemInfoService::GetTotalMemoryBytes() {
        ...
        if (initResult.HasValue()) {
            ...
            if (memoryResult.HasValue()) {
                ...
                if (memory > 0) {
                    if (m_logger) {
                        m_logger->Debug(
                            L"Total memory from SMBIOS: " +
                            std::to_wstring(memory / (1024 * 1024)) +
                            L" MB"
                        );
                    }
                    return memory;
        ...
        if (!GlobalMemoryStatusEx(&memStatus)) {
            ...
            if (m_logger) {
                m_logger->Error(
                    L"Failed to get memory status, error code: " +
                    std::to_wstring(error)
                );
            }
            return domain::Error{
                L"Failed to get total memory",
                error,
                domain::ErrorCategory::System
            };
        }
        ...
        if (m_logger) {
            m_logger->Info(
                L"Total physical memory: " +
                std::to_wstring(totalMemory / (1024 * 1024)) +
                L" MB"
            );
        }
