...
namespace winsetup::adapters::persistence {
    ...
    domain::Expected<std::shared_ptr<domain::SetupConfig>>
        ...
        if (data.find(L"USERPROFILE") != data.end()) {
            auto result = ParseUserProfile(data[L"USERPROFILE"], *config);
            if (!result.HasValue()) {
        ...
        if (data.find(L"PARTITION") != data.end()) {
            auto result = ParsePartition(data[L"PARTITION"], *config);
            if (!result.HasValue()) {
                return result.GetError();
            }
        }
        ...
        if (data.find(L"BACKUP") != data.end()) {
            auto result = ParseBackup(data[L"BACKUP"], *config);
            if (!result.HasValue()) {
                return result.GetError();
            }
        }
        ...
        if (data.find(L"TYPES") != data.end()) {
            auto result = ParseTypes(data[L"TYPES"], *config);
            if (!result.HasValue()) {
                return result.GetError();
            }
        }
        ...
        if (data.find(L"TIMES") != data.end()) {
            auto result = ParseTimes(data[L"TIMES"], *config);
            if (!result.HasValue()) {
                return result.GetError();
            }
        }
    ...
    domain::Expected<void>
        ...
        return domain::Error(
            L"SaveConfig not implemented",
            0,
            domain::ErrorCategory::NotImplemented
        );
    }

    domain::Expected<void> IniConfigRepository::ParseUserProfile(
    ...
    domain::Expected<void> IniConfigRepository::ParsePartition(
        const std::map<std::wstring, std::wstring>& section,
        domain::SetupConfig& config
    ) {
        auto it = section.find(L"DATAPARTITIOM");
        if (it != section.end()) {
    ...
    domain::Expected<void> IniConfigRepository::ParseBackup(
        const std::map<std::wstring, std::wstring>& section,
        domain::SetupConfig& config
    ) {
    ...
    domain::Expected<void> IniConfigRepository::ParseTypes(
        const std::map<std::wstring, std::wstring>& section,
        domain::SetupConfig& config
    ) {
    ...
    domain::Expected<void> IniConfigRepository::ParseTimes(
        const std::map<std::wstring, std::wstring>& section,
        domain::SetupConfig& config
    ) {
        for (const auto& [key, value] : section) {
